...
const App = () => {
    ...
    useEffect(() => {
        const max = Math.max(...selectedVotes);
        if (max > 0) {
            const maxVotes = selectedVotes.indexOf(max)
    ...
    return (
        <div>
            ...
            <br/>
            <button onClick={() => {
                const copy = [...selectedVotes]
                copy[selected] += 1
                setSelectedVotes(copy)
            } }> vote</button>
            <button onClick={() => setSelected(Math.floor(Math.random() * anecdotes.length))}> next anecdote</button>
            <br/>
            <h1>Anecdote with most votes</h1>
            { mostVotes !== -1 ? anecdotes[mostVotes] : 'No votes yet' }
        </div>
    )
}
